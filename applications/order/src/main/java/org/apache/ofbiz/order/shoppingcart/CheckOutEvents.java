...
public class CheckOutEvents {
    ...
    public static boolean explodeOrderItems(Delegator delegator, ShoppingCart cart) {
        ...
        return !(productStore == null || productStore.get(x.explodeOrderItems) == null)
                && productStore.getBoolean(x.explodeOrderItems);
    }
    ...
    public static String checkShipmentNeeded(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            Debug.logInfo("checkShipmentNeeded: reqShipAddrForDigItems=" + productStore.getString(x.reqShipAddrForDigItems), MODULE);
        } catch (GenericEntityException e) {
            Debug.logError(e, "Error getting ProductStore: " + e.toString(), MODULE);
        }

        if (productStore != null && "N".equals(productStore.getString(x.reqShipAddrForDigItems))) {
    ...
    public static String failedDenylistCheck(HttpServletRequest request, HttpServletResponse response) {
        ...
        // wipe the session
        if (("anonymous".equals(currentUser.getString(x.userLoginId))) || (currentUser.getString(x.userLoginId))
                .equals(userLogin.getString(x.userLoginId))) {
            session.invalidate();
        }
        ...
        if ("EXT_PAYPAL".equals(paymentMethodTypeId) || cart.getPaymentMethodTypeIds().contains("EXT_PAYPAL")) {
            try {
                GenericValue payPalProdStorePaySetting = EntityQuery.use(delegator).from("ProductStorePaymentSetting").where("productStoreId",
                        productStore.getString(x.productStoreId), "paymentMethodTypeId", "EXT_PAYPAL").queryFirst();
                if (payPalProdStorePaySetting != null) {
                    GenericValue gatewayConfig = payPalProdStorePaySetting.getRelatedOne(x.PaymentGatewayConfig, false);
                    if (gatewayConfig != null && "PAY_GATWY_PAYFLOWPRO".equals(gatewayConfig.getString(x.paymentGatewayConfigTypeId))) {
                        return "paypal";
                    }
                }
            } catch (GenericEntityException e) {
        ...
        if ("cust".equals(mode)) {
            ...
            if (partyId != null) {
                ...
                if (userLogin == null) {
                    ...
                    if (userLogin != null) {
                        userLogin.set(x.partyId, partyId);
                    }
    ...
    public static String createReplacementOrder(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (grandTotal.compareTo(new BigDecimal(0)) != 0) {
            ...
            adjustment.set(x.orderAdjustmentTypeId, "REPLACE_ADJUSTMENT");
            adjustment.set(x.amount, grandTotal.negate());
            adjustment.set(x.comments, "ReShip Order for Order #" + originalOrderId);
            adjustment.set(x.createdDate, UtilDateTime.nowTimestamp());
            adjustment.set(x.createdByUserLogin, userLogin.getString(x.userLoginId));
        ...
        for (ShoppingCartItem sci : cartLines) {
            ...
            try {
                GenericValue orderItem = EntityQuery.use(delegator).from("OrderItem")
                        ...
                        .queryFirst();
                if (orderItem != null) {
                    sci.setAssociatedOrderId(orderItem.getString(x.orderId));
                    sci.setAssociatedOrderItemSeqId(orderItem.getString(x.orderItemSeqId));
