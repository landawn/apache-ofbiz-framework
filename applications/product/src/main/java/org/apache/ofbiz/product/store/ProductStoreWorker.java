...
public final class ProductStoreWorker {
    ...
    public static String getProductStoreId(ServletRequest request) {
        ...
        if (session != null && session.getAttribute("productStoreId") != null) {
            ...
            if (webSite != null) {
                String productStoreId = webSite.getString(x.productStoreId);
    ...
    public static String getStoreCurrencyUomId(HttpServletRequest request) {
        ...
        if (UtilValidate.isEmpty(productStore)) {
            ...
            return UtilHttp.getCurrencyUom(request.getSession(), productStore.getString(x.defaultCurrencyUomId));
        }
    }
    ...
    public static Locale getStoreLocale(HttpServletRequest request) {
        ...
        if (UtilValidate.isEmpty(productStore)) {
            ...
            return UtilHttp.getLocale(request, request.getSession(), productStore.getString(x.defaultLocaleString));
        }
    }
    ...
    public static TimeZone getStoreTimeZone(HttpServletRequest request) {
        ...
        if (UtilValidate.isEmpty(productStore)) {
            ...
            return UtilHttp.getTimeZone(request, request.getSession(), productStore.getString(x.defaultTimeZoneString));
        }
    }
    ...
    public static String determineSingleFacilityForStore(Delegator delegator, String productStoreId) {
        ...
        if (productStore != null) {
            if ("Y".equalsIgnoreCase(productStore.getString(x.oneInventoryFacility))) {
                return productStore.getString(x.inventoryFacilityId);
            }
        }
        return null;
    }
    ...
    public static boolean autoSaveCart(GenericValue productStore) {
        return productStore == null ? false : "Y".equalsIgnoreCase(productStore.getString(x.autoSaveCart));
    }
    ...
    public static String getProductStorePaymentProperties(Delegator delegator, String productStoreId, String paymentMethodTypeId,
        ...
        if (setting != null && setting.get(x.paymentPropertiesPath) != null) {
            payProps = setting.getString(x.paymentPropertiesPath);
        }
        return payProps;
    }
    ...
    public static List<GenericValue> getAvailableStoreShippingMethods(Delegator delegator, String productStoreId, GenericValue shippingAddress,
        ...
        if (shippingMethods != null) {
            for (GenericValue method: shippingMethods) {
                ...
                BigDecimal minWeight = method.getBigDecimal(x.minWeight);
                BigDecimal maxWeight = method.getBigDecimal(x.maxWeight);
                ...
                // test order total
                BigDecimal minTotal = method.getBigDecimal(x.minTotal);
                BigDecimal maxTotal = method.getBigDecimal(x.maxTotal);
                ...
                // test product sizes
                BigDecimal minSize = method.getBigDecimal(x.minSize);
                BigDecimal maxSize = method.getBigDecimal(x.maxSize);
                ...
                // check USPS address
                String allowUspsAddr = method.getString(x.allowUspsAddr);
                String requireUspsAddr = method.getString(x.requireUspsAddr);
                ...
                String companyPartyId = method.getString(x.companyPartyId);
                String allowCompanyAddr = method.getString(x.allowCompanyAddr);
                String requireCompanyAddr = method.getString(x.requireCompanyAddr);
                ...
                String includeFreeShipping = method.getString(x.includeNoChargeItems);
                ...
                // check the geos
                String includeGeoId = method.getString(x.includeGeoId);
                String excludeGeoId = method.getString(x.excludeGeoId);
                ...
                if (UtilValidate.isNotEmpty(includeGeoId)) {
                    ...
                    if (!GeoWorker.containsGeo(includeGeoGroup, shippingAddress.getString(x.countryGeoId), delegator)
                            && !GeoWorker.containsGeo(includeGeoGroup, shippingAddress.getString(x.stateProvinceGeoId), delegator)
                            && !GeoWorker.containsGeo(includeGeoGroup, shippingAddress.getString(x.postalCodeGeoId), delegator)) {
                ...
                if (UtilValidate.isNotEmpty(excludeGeoId)) {
                    ...
                    if (GeoWorker.containsGeo(excludeGeoGroup, shippingAddress.getString(x.countryGeoId), delegator)
                            || GeoWorker.containsGeo(excludeGeoGroup, shippingAddress.getString(x.stateProvinceGeoId), delegator)
                            || GeoWorker.containsGeo(excludeGeoGroup, shippingAddress.getString(x.postalCodeGeoId), delegator)) {
                        // in excluded geos
                        returnShippingMethods.remove(method);
                        continue;
                    }
                ...
                // check the features
                String includeFeatures = method.getString(x.includeFeatureGroup);
                String excludeFeatures = method.getString(x.excludeFeatureGroup);
                    ...
                    if (includedFeatures != null
                            && (featureIdMap == null
                                || includedFeatures.stream().noneMatch(
                                    appl -> featureIdMap.containsKey(appl.getString(x.productFeatureId))))) {
                        returnShippingMethods.remove(method);
                        continue;
                    }
                }
                if (UtilValidate.isNotEmpty(excludeFeatures)) {
                    ...
                    if (excludedFeatures != null && featureIdMap != null
                            && excludedFeatures.stream().anyMatch(appl -> featureIdMap.containsKey(appl.getString(x.productFeatureId)))) {
                        returnShippingMethods.remove(method);
                    }
                }
            }
    ...
    public static ProductStoreSurveyWrapper getRandomSurveyWrapper(HttpServletRequest request, String groupName) {
        ...
        String partyId = userLogin != null ? userLogin.getString(x.partyId) : null;
        ...
        return getRandomSurveyWrapper(productStore.getDelegator(), productStore.getString(x.productStoreId), groupName, partyId, passThruFields);
    }
        ...
        if (UtilValidate.isNotEmpty(productId) && UtilValidate.isNotEmpty(storeSurveys)) {
            for (GenericValue surveyAppl: storeSurveys) {
                ...
                try {
                    ...
                    if ((product != null) && ("Y".equals(product.get(x.isVariant)))) {
                ...
                if (surveyAppl.get(x.productId) != null) {
                    if (surveyAppl.get(x.productId).equals(productId)) {
                        surveys.add(surveyAppl);
                    } else if ((virtualProductId != null) && (surveyAppl.getString(x.productId).equals(virtualProductId))) {
                        surveys.add(surveyAppl);
                    }
                } else if (surveyAppl.get(x.productCategoryId) != null) {
                    ...
                    try {
                        categoryMembers = EntityQuery.use(delegator).from("ProductCategoryMember").where("productCategoryId",
                                surveyAppl.get(x.productCategoryId)).cache(true).queryList();
                    } catch (GenericEntityException e) {
                    ...
                    if (categoryMembers != null) {
                        for (GenericValue member: categoryMembers) {
                            if (productId != null && productId.equals(member.getString(x.productId))) {
                                surveys.add(surveyAppl);
                                break;
                            } else if ((virtualProductId != null) && (virtualProductId.equals(member.getString(x.productId)))) {
    ...
    public static int checkSurveyResponse(HttpServletRequest request, String surveyId) {
        ...
        return checkSurveyResponse(delegator, userLogin.getString(x.partyId), productStoreId, surveyId);
    }
        ...
        try {
            ...
            if (wantRequired != null) {
                Map<String, Object> invReqResult = dispatcher.runSync("isStoreInventoryRequired",
                        UtilMisc.toMap("productStoreId", productStoreId, "productId", product.get(x.productId), "product", product,
    ...
    public static boolean isStoreInventoryAvailable(String productStoreId, ProductConfigWrapper productConfig, BigDecimal quantity,
        ...
        if ("N".equals(productStore.getString(x.checkInventory))) {
        ...
        if ("Y".equals(productStore.getString(x.oneInventoryFacility))) {
            String inventoryFacilityId = productStore.getString(x.inventoryFacilityId);
            ...
            if (UtilValidate.isNotEmpty(productFacilities)) {
                for (GenericValue pfValue: productFacilities) {
                    isInventoryAvailable = ProductWorker.isProductInventoryAvailableByFacility(productConfig,
                            pfValue.getString(x.facilityId), quantity, dispatcher);
