...
public class IssuanceTest extends OFBizTestCase {
    ...
    public void testMultipleInventoryItemIssuance() throws Exception {
        ...
        List<GenericValue> orderShipments = orderHeader.getRelated(x.OrderShipment, null, null, false);
        ...
        assertEquals(orderItemSeqId, orderShipment.getString(x.orderItemSeqId));
        assertEquals(shipGroupSeqId, orderShipment.getString(x.shipGroupSeqId));
        assertEquals(shipmentId, orderShipment.getString(x.shipmentId));
        assertEquals(shipmentItemSeqId, orderShipment.getString(x.shipmentItemSeqId));
        BigDecimal actual = orderShipment.getBigDecimal(x.quantity);
        ...
        List<GenericValue> itemIssuances = orderHeader.getRelated(x.ItemIssuance, null, UtilMisc.toList("inventoryItemId"), false);
        ...
        assertEquals(orderItemSeqId, itemIssuance.getString(x.orderItemSeqId));
        assertEquals(shipGroupSeqId, itemIssuance.getString(x.shipGroupSeqId));
        assertEquals(shipmentId, itemIssuance.getString(x.shipmentId));
        assertEquals(shipmentItemSeqId, itemIssuance.getString(x.shipmentItemSeqId));
        assertEquals("9001", itemIssuance.getString(x.inventoryItemId));
        actual = itemIssuance.getBigDecimal(x.quantity);
        ...
        assertEquals(shipmentId, itemIssuance.getString(x.shipmentId));
        assertEquals(shipmentItemSeqId, itemIssuance.getString(x.shipmentItemSeqId));
        assertEquals("9025", itemIssuance.getString(x.inventoryItemId));
        actual = itemIssuance.getBigDecimal(x.quantity);
        ...
        List<GenericValue> reservations = orderHeader.getRelated(x.OrderItemShipGrpInvRes, null, null, false);
        ...
        assertEquals(orderHeader.getString(x.statusId), "ORDER_COMPLETED");
        ...
        for (GenericValue orderItem : orderItems) {
            assertEquals("ITEM_COMPLETED", orderItem.getString(x.statusId));
        }
    }
}
