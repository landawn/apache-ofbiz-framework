...
public class ProposedOrder {
    ...
    public Map<String, Object> calculateStartDate(int daysToShip, GenericValue routing, Delegator delegator, LocalDispatcher dispatcher,
        ...
        if (isBuilt) {
            ...
            if (routing == null) {
                try {
                    Map<String, Object> routingInMap = UtilMisc.<String, Object>toMap("productId", product.getString(x.productId),
                    ...
                    if (routing == null) {
                        ...
                        try {
                            tree = new BOMTree(product.getString(x.productId), "MANUF_COMPONENT", requiredByDate,
            ...
            if (routing != null) {
                ...
                if (listRoutingTaskAssoc == null) {
                    try {
                        Map<String, Object> routingTasksInMap = UtilMisc.<String, Object>toMap("workEffortId", routing.getString(x.workEffortId),
                                "userLogin", userLogin);
            ...
            if (listRoutingTaskAssoc != null) {
                for (int i = 1; i <= listRoutingTaskAssoc.size(); i++) {
                    ...
                    if (EntityUtil.isValueActive(routingTaskAssoc, endDate)) {
                        ...
                        try {
                            routingTask = routingTaskAssoc.getRelatedOne(x.ToWorkEffort, true);
                        ...
                        result.put(routingTask.getString(x.workEffortId), startDate);
                        endDate = startDate;
                    }
                }
            } else {
                // routing is null
                Debug.logError("No routing found for product = " + product.getString(x.productId), MODULE);
            }
        } else {
    ...
    public String create(DispatchContext ctx, GenericValue userLogin) {
        if ("WIP".equals(product.getString(x.productTypeId))) {
