...
public class LoopSubContentTransform implements TemplateTransformModel {
    ...
    public static boolean prepCtx(Delegator delegator, Map<String, Object> ctx) {
        ...
        try {
            electronicText = subContentDataResourceView.getRelatedOne(x.ElectronicText, false);
        } catch (GenericEntityException e) {
            throw new RuntimeException(e.getMessage());
        }

        String dataResourceId = (String) subContentDataResourceView.get(x.drDataResourceId);
        String subContentIdSub = (String) subContentDataResourceView.get(x.contentId); // in ContentAssocDataResourceViewTo
        // This order is taken so that the dataResourceType can be overridden in the transform arguments.
        String subDataResourceTypeId = (String) ctx.get("subDataResourceTypeId");
        if (UtilValidate.isEmpty(subDataResourceTypeId)) {
            subDataResourceTypeId = (String) subContentDataResourceView.get(x.drDataResourceTypeId);
        ...
        if (UtilValidate.isEmpty(mimeTypeId)) {
            mimeTypeId = (String) subContentDataResourceView.get(x.mimeTypeId);
        ...
        if (electronicText != null) {
            ctx.put("textData", electronicText.get(x.textData));
        } else {
