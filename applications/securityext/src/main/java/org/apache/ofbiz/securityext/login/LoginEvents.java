...
public class LoginEvents {
    ...
    public static String showPasswordHint(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (supposedUserLogin != null) {
            passwordHint = supposedUserLogin.getString(x.passwordHint);
        }
    ...
    public static String emailPasswordRequest(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            userLogin = EntityQuery.use(delegator)
                    ...
                    .queryOne();
            if (userLogin == null || "N".equals(userLogin.getString(x.enabled))) {
            ...
            GenericValue userParty = userLogin.getRelatedOne(x.Party, false);
            if (userParty == null) {
            ...
            if (productStoreEmail != null) {
                bodyScreenLocation = productStoreEmail.getString(x.bodyScreenLocation);
            }
            ...
            if (productStoreEmail != null) {
                serviceContext.put("subject", productStoreEmail.getString(x.subject));
                serviceContext.put("sendFrom", productStoreEmail.get(x.fromAddress));
                serviceContext.put("sendCc", productStoreEmail.get(x.ccAddress));
                serviceContext.put("sendBcc", productStoreEmail.get(x.bccAddress));
                serviceContext.put("contentType", productStoreEmail.get(x.contentType));
            } else {
                ...
                if (emailTemplateSetting != null) {
                    String subject = emailTemplateSetting.getString(x.subject);
                    subject = FlexibleStringExpander.expandString(subject, UtilMisc.toMap("userLoginId", userLoginId));
                    serviceContext.put("subject", subject);
                    serviceContext.put("sendFrom", emailTemplateSetting.get(x.fromAddress));
                } else {
            ...
            serviceContext.put("partyId", userParty.getString(x.partyId));
