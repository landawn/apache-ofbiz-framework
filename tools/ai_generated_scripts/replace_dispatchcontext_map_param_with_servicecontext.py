...
# only replace second parameter after DispatchContext in signatures/candidates
PARAM_PATTERN = re.compile(r"(DispatchContext\s+[A-Za-z_][A-Za-z0-9_]*\s*,\s*)Map\s*<\s*String\s*,\s*Object\s*>\s+context")
...
for path in ROOT.rglob("*Services.java"):
    ...
    new_text, count = PARAM_PATTERN.subn(r"\1ServiceContext context", text)
    if count == 0:
        continue
